      ...
      <section className="flex justify-center bg-primary-50 bg-dotted-pattern bg-contain">
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl">
          ...
          <div className="flex w-full flex-col gap-8 p-5 md:p-10">
            ...
            {isEventCreator && (
              <Link to={`/events/${event._id}/tasks`}  className="bg-purple-600 hover:bg-purple-500 text-white rounded-full px-4 py-2 w-40 text-center text-sm sm:text-base">
                Manage Tasks
              </Link>
            )}
      ...
      <section className="wrapper my-8 flex flex-col gap-8 md:gap-12">
        ...
        <div className="flex flex-row flex-wrap gap-8 justify-center items-center">
          {relatedEvents.length === 1 ? (
            <h3 className="h3-bold p-48">No Related Events</h3>
          ) : (
            relatedEvents
              .filter((relatedEvent) => relatedEvent._id !== event._id)
              .map((relatedEvent) => <EventCard key={relatedEvent._id} event={relatedEvent} />)
          )}
        </div>
      </section>
    </>
